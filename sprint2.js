...
/*1*/
function display(param) {console.log(param)}
...
let esperarSaludar = (nombre, callFunction) => setTimeout(() => callFunction(nombre), 2000)
...
let procesarElements = (arrElements, callF) => arrElements.forEach(element => callF(element))
...
const result2 = arr5.filter(n => n >= 10).map(n => n * 2).reduce((acc, n) =>{
    return acc + n
}, 0)
...
/*5 - FOR-OF con INDEX, iterar para buscar*/
